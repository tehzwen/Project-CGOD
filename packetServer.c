...
int init_server(struct sockaddr_in *svaddr)
{
	int sockfd = socket(AF_INET, SOCK_DGRAM, 0);
	struct sockaddr_in servaddr = *svaddr;
...
int main(int argc, char *argv[])
{
	...
	puts("Network initialization successful");
	...
	while (1)
	{
		...
		inet_ntop(AF_INET, &cliaddr, client_addr, 20);
		puts(client_addr);
